...
把正式部署從原 Python 版（`/home/joe/文件/docker/qwen2API`，端口 7860）切換到本 Rust 版。

- **源碼**：`/home/joe/文件/dev/qwen2api-rs`（開發在此）
- **部署資料夾**：`/home/joe/文件/docker/qwen2api-rs`（只放 compose + data；源碼用 build context 引用 dev）
- **對外端口**：7860（取代原版）
- **上游出口**：默认直连；如确需指定出口，只设置 `UPSTREAM_PROXY`。通用
  `HTTP_PROXY/HTTPS_PROXY/ALL_PROXY` 会被忽略，避免被容器或宿主环境隐式切到 WAF 出口。
...
services:
  qwen2api-rs:
    ...
    environment:
      ...
      # 上游出口代理默认不启用。确实需要固定出口时只设置 UPSTREAM_PROXY。
...
> qwen2api-rs 会强制忽略 `HTTP_PROXY/HTTPS_PROXY/ALL_PROXY`，避免隐式代理导致
> Qwen WAF。cutover 前如需测试代理链路，请使用 `UPSTREAM_PROXY=... ./qwen2api-rs`。
